...
func ListNotifications(userID string , limit int, hub *Hub, w http.ResponseWriter) {
	...
	if err:= GetTotalCount(&totalCount, hub, w); err != nil {
		return
	}
...
func GetTotalCount(totalCount *int64, hub *Hub, w http.ResponseWriter) error {
...
func GetUnreadCount(unreadCount *int64, hub *Hub, w http.ResponseWriter, userID string) error{
	...
	Where("NOT EXISTS (SELECT notification_id FROM notification_users WHERE notified_id = ?)", userID).
