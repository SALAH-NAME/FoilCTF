...
import {
	...
	teams as table_teams,
	team_members as table_team_members,
	team_join_requests as table_team_join_requests,
	notifications as table_notifications,
	notification_users as table_notification_users,
} from './db/schema';
...
export const createTeam = async (
	...
	await db.transaction(async (tx) => {
		...
		if (!dbUser) throw new FoilCTF_Error('Forbidden', 403);
		if (dbUser.team_name)
		...
		if (existingTeam) throw new FoilCTF_Error('Team name already used', 409);

		await tx
			.insert(table_teams)
			.values({ name, captain_name: captain_user.username });
		await tx
			.insert(table_team_members)
			.values({ team_name: name, member_name: captain_user.username });
		await tx
			.update(table_users)
			.set({ team_name: name })
			.where(eq(table_users.id, captain_user.id));
...
export const getTeamMembers = async (req: Request, res: Response) => {
	const members = await db
		...
		.leftJoin(
			table_users,
			eq(table_users.username, table_team_members.member_name)
		)
		.leftJoin(
			table_profiles,
			eq(table_profiles.username, table_team_members.member_name)
		)
...
export const leaveTeam = async (
	...
	try {
		await db.transaction(async (tx) => {
			...
			if (
				team.captain_name === decodedUser.username &&
			...
			const [updatedTeam] = await tx
				...
				.returning();
			if (!updatedTeam) throw new Error('Could not update DB');
			await tx
				.update(table_users)
				.set({ team_name: null })
				.where(eq(table_users.username, decodedUser.username));
			if (updatedTeam.members_count === 0) {
...
export const handOverLeadership = async (
	...
	await db.transaction(async (tx) => {
		...
		if (!member) throw new FoilCTF_Error('Forbidden', 403);

		await tx
			.update(table_teams)
			.set({ captain_name: target_username })
			.where(eq(table_teams.name, team.name));
...
export const sendJoinRequest = async (
	...
	await db.transaction(async (tx) => {
		...
		if (!request_team) throw new FoilCTF_Error('Not Found', 404);
		...
		await tx.insert(table_team_join_requests).values({
...
export const cancelJoinRequest = async (
	...
	if (typeof team_name !== 'string' || !team_name)
		return res
			.status(404)
			.json(new FoilCTF_Error('Invalid team name', 404))
			.end();
...
export const acceptJoinRequest = async (
	...
	await db.transaction(async (tx) => {
		...
		const [team] = await tx
			...
			.from(table_teams)
			.where(eq(table_teams.captain_name, captain_user.username));
		if (!team || team.name !== target_team_name || team.is_locked) {
			throw new FoilCTF_Error('Forbidden', 403);
		}
...
export const notifyCaptain = async (
	_req: Request,
	res: Response,
	_next: NextFunction
) => {
	...
	try {
		await db.transaction(async (tx) => {
			...
			await tx.insert(table_notification_users).values(notificationUserRow); // TODO(xenobas): Continue the work on camelCase -> snake_case
			await tx
				.update(table_notifications)
				.set({ is_published: true })
...
export const notifyAllMembers = async (
	...
	try {
		await db.transaction(async (tx) => {
			...
			await tx.insert(table_notification_users).values(notification_user_rows);
			await tx
				.update(table_notifications)
				.set({ is_published: true })
				.where(eq(table_notifications.id, insertedNotification.id));
...
export const route_team_delete = async (
	req: Request,
	res: Response<any, { user: JWT_Payload }>
) => {
	const captain_user = res.locals.user;
...
export const route_team_requests = async (req: Request, res: Response) => {
	...
	const [team] = await db
		...
		.from(table_teams)
		.where(eq(table_teams.captain_name, decodedUser.username));
	if (!team) return res.status(403).json(new FoilCTF_Error('Forbidden', 403));

	const where_and_conditions = [
		eq(table_team_join_requests.team_name, team.name),
	];
	if (search)
		where_and_conditions.push(
			ilike(table_team_join_requests.username, `%${search}%`)
		);
...
export const route_team_me = async (
	_req: Request,
	res: Response<any, { user: JWT_Payload }>
) => {
	const user = res.locals.user;
	const team = await db.transaction(async (tx) => {
		const [member_user] = await tx
			.select({ team_name: table_users.team_name })
			.from(table_users)
		...
		if (!team) {
			console.error(
				'ERROR - Invariant broken, User team no longer exists, yet team_name column still contains "%s"',
				member_user.team_name
			);
