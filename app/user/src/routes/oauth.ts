...
import { ENV_OAUTH_42_SECRET, ENV_OAUTH_42_UID } from '../utils/env';
...
export function route_oauth_42_connect(req: Request, res: Response) {
	...
	const uri_redirect = new URL("/api/oauth/42/connect/verify", `${uri_service_protocol}://${uri_service_origin}`);
...
export async function route_oauth_42_link(req: Request, res: Response<any, { user: User }>) {
	const { user: { id: user_id } } = res.locals;

	const [user] = await db.select().from(table_users).where(eq(table_users.id, user_id));
	if (user!.oauth42_login)
		return res.status(400).send('User already is linked to a 42Intra profile').end();

	const uri_redirect_client = zod.url().parse(req.query.redirect_uri);
	console.log('DEBUG - OAuth42 - Verify -> "%s"', uri_redirect_client);

	const uri_service_protocol = req.protocol;
	const uri_service_origin = zod.string().parse(req.get("Host"));

	const uri_redirect = new URL("/api/oauth/42/link/verify", `${uri_service_protocol}://${uri_service_origin}`);
	uri_redirect.searchParams.set("redirect_uri", uri_redirect_client)
...
async function fetch_42_token(req: Request, route_origin: 'connect' | 'link', uri_redirect_client: string, code: string, user_id?: number): Promise<string | null>  {
	const uri_service_protocol = req.protocol;
	const uri_service_origin = zod.string().parse(req.get("Host"));

	const uri_redirect = new URL(`/api/oauth/42/${route_origin}/verify`, `${uri_service_protocol}://${uri_service_origin}`);
	uri_redirect.searchParams.set("redirect_uri", uri_redirect_client)
	if (user_id)
		uri_redirect.searchParams.set('user_id', user_id.toString());
	...
	if (!res_token.ok) {
		...
		console.log('ERROR - OAuth42 - /oauth/token status code %d, %s', res_token.status, res_token.statusText);
		console.log(res_token.headers.get('content-type') === 'application/json' ? JSON.parse(text_token) : text_token);
		return (null);
	}
...
async function fetch_42_profile(token: string): Promise<zod.infer<typeof schema_42_profile> | null> {
...
export function route_oauth_42_verify(route_origin: 'connect' | 'link') {
	...
	const schema_query = zod.intersection(schema_query_shared, zod.union([schema_query_success, schema_query_failure]));

	return async (req: Request, res: Response) => {
		const { redirect_uri, user_id, ...query } = schema_query.parse(req.query);
		console.log('DEBUG - OAuth42 - Verify -> "%s"', redirect_uri);

		if (route_origin === 'link' && user_id === undefined) {
			const uri = new URL(redirect_uri);
			uri.searchParams.set('error', 'query-missing');
			uri.searchParams.set('error_description', 'Required parameter "user_id" is missing');

			return res.redirect(303, uri.toString());
		}
		if (Object.hasOwn(query, 'error')) {
			const { error, error_description } = query as zod.infer<typeof schema_query_failure>;
		...
		const token = await fetch_42_token(req, route_origin, redirect_uri, code, user_id);
		if (!token) {
			const uri = new URL(redirect_uri);
			uri.searchParams.set('error', 'oauth-token');
			uri.searchParams.set('error_description', 'Could not fetch 42OAuth token');
		...
		if (!profile) {
			...
			uri.searchParams.set('error_description', 'Could not fetch 42Intra profile');

			return res.redirect(303, uri.toString());
		}

		const uri = new URL(redirect_uri);
		uri.searchParams.set('oauth_profile', base64_encode(profile));
		if (route_origin === 'link') try {
			await db
				.update(table_users)
			...
			uri.searchParams.set('error_description', 'Could not link OAuth profile with user');
