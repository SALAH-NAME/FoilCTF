...
export async function remote_fetch_events(
	q: string,
...
export default function Page({ loaderData }: Route.ComponentProps) {
	...
	const query_events = useQuery({
		...
		queryFn: async () => {
			...
			return await remote_fetch_events(
				searchTerm,
				activeFilter,
				sortOrder,
				currentPage,
	...
	const {
		data: { events, count: pagination_count_elements },
	} = query_events;
	...
	const pagination_count_pages = Math.max(
		1,
		Math.ceil(pagination_count_elements / itemsPerPage)
	);
	...
	useEffect(() => {
		...
		const valid_filter =
			param_filter && Object.keys(OEventStatus).includes(param_filter);
