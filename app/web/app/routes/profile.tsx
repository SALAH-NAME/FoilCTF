...
async function update_password(
	...
	const res = await fetch(uri, {
		...
		body: JSON.stringify({ password_new, password }),
	});
...
export default function Page({ loaderData, actionData }: Route.ComponentProps) {
	...
	useEffect(() => {
		const error = queryUser.error;
		if (!error) return;

		addToast({
			variant: 'error',
			title: 'Could not query user',
			message: error.message || 'Internal server error',
		});
		console.error(error);
	}, [queryUser.errorUpdateCount, queryUser.errorUpdatedAt]);
	...
	useEffect(() => {
		if (actionData?.error || !actionData?.timestamp) return;
	...
	const handlePasswordFormSubmit = (e: SubmitEvent<HTMLFormElement>) => {
		const validations = [
			validationRules.password(passwordForm.currentPassword),
			validationRules.password(passwordForm.newPassword),
